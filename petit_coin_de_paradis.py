...
def imports():
    ...
    scores =pd.read_csv("file:///Users/timothee/Desktop/Wild%20Code%20School/drive-download-20201120T125440Z-001/datamaquereau.csv",index_col='Code_commune')
    scores.drop(columns="COM",inplace=True)

    coordonnees = pd.read_csv('file:///Users/timothee/Desktop/Wild%20Code%20School/drive-download-20201120T125440Z-001/databrème%20(1).csv')
    coordonnees.set_index('Code_commune', inplace = True)
    return scores, coordonnees

scores, coordonnees = imports()
...
scores=scores.loc[scores.index.isin(coordonnees.index)]

def geo(ind):
    return coordonnees.loc[coordonnees.index.isin(ind)]

a=user_result(scores,5,euclid)
